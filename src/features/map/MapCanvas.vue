...
function updateTiles() {
  ...
  if (tileLayer && tileEra === props.era) {
    tileUrl = url
...
function updateMarkers() {
  ...
  const visibleKeys = new Set(props.items.map(item => item.markerKey))

  for (const [key, entry] of markers) {
    if (!visibleKeys.has(key) && markerLayer.hasLayer(entry.marker)) {
      markerLayer.removeLayer(entry.marker)
    }
  }

  for (const item of props.items) {
    let entry = markers.get(item.markerKey)

    if (!entry) {
      entry = { item, marker: L.marker(item.coords, { icon: makeIcon(item) }) }
      entry.marker.bindPopup(makePopup(item), {
        autoPan: false,
        className: 'custom-leaflet-popup',
      })
      entry.marker.on('click', () => emit('markerSelect', entry.item))
      markers.set(item.markerKey, entry)
    } else if (entry.item.index !== item.index) {
      entry.marker.setIcon(makeIcon(item))
      entry.marker.setPopupContent(makePopup(item))
    }
...
<template>
  <div class="map-canvas-container">
    <div
      ...
      class="map-canvas"
      role="region"
      aria-label="Carte interactive de Lyon"
    />
    <p
      v-if="tileStatus === 'loading'"
      class="map-load-message"
      role="status"
    >
      Chargement du fond cartographique…
    </p>
    <p
      v-else-if="tileStatus === 'error'"
      class="map-load-message"
      role="alert"
    >
